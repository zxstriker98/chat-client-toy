...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Handle placeholder physical_index values (e.g., "<physical_index_1>")
    if isinstance(last_physical_index, str) and last_physical_index.startswith('<physical_index'):
        last_physical_index = None
    if last_physical_index is None or (isinstance(last_physical_index, (int, float)) and last_physical_index < len(page_list)/2):
        return 0, []
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        return toc_with_page_number
    # If accuracy is 0 or low, try fallback modes
    if accuracy == 0 and mode == 'process_toc_with_page_numbers':
        print(f'[DEBUG] Accuracy 0 - falling back to process_toc_no_page_numbers')
